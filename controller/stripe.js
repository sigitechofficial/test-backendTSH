...
async function addCard(
  ...
  try {
    // const {customerId,cardName,cardExpYear,cardExpMonth,cardNumber,cardCVC,} = data;
...
async function cards(customerId) {
  try {
    ...
    console.log(
      "🚀 ~ file: stripe.js:58 ~ cards ~ paymentMethods:",
      paymentMethods
    );
...
async function createSubscriptionWithPriceId(customerId, priceId) {
  try {
    ...
    console.log(
      "ðŸš€ ~ createSubscriptionWithPriceId ~ subscription.latest_invoice.payment_intent:",
...
async function retrieveCheckoutSession(sessionId) {
  ...
  //console.log("🚀 ~ retrieveCheckoutSession ~ checkoutSession:", checkoutSession.payment_status)
...
async function retriveIntent(intentId) {
  console.log("🚀 ~ retriveIntent ~ intentId:", typeof(intentId))

  try {
    if (!intentId || typeof intentId !== 'string') {
      throw new CustomException('Invalid PaymentIntent ID');
    }

    // Retrieve the PaymentIntent from Stripe
    const intent = await stripe.paymentIntents.retrieve(intentId);
    //console.log("🚀 ~ retriveIntent ~ intent:", intent)
